...
We define all reconstructed $\gamma$ pairs as background, where the corresponding true particles cannot be traced back to a common $\pi^0$ parent (or $\eta$ when we want to reconstruct those, but in this channel the backgrounds are small). 
As shown below, a common occurence is that one of the decay photons decays via $\gamma \rightarrow e^+e^-$ and we identify one of the leptons as a photon. We define the thus reconstructed neutral meson as part of the signal. 
...
\begin{figure}[H]
  \centering     
  \subfigure[$\pi^0$ component for $0.2<z<0.3$ ]{\label{fig:mc_singlez_component}\includegraphics[width=0.8\textwidth,natwidth=600,natheight=400]{figure_dataselection/pi0_component_Z_1.pdf}}
  \subfigure[$\pi^0$ component for $0.4<z<0.5$ ]{\label{fig:mc_singlept_component}\includegraphics[width=0.8\textwidth,natwidth=600,natheight=400]{figure_dataselection/pi0_component_Z_3.pdf}}
...
\end{figure}
As shown in Fig.~\ref{fig:pi0_component}, the dominant background is combinatorial originating from different $\pi^0$. Because both parent mesons carry a Collins asymmetry, it is possible that the asymmetry for this background is also non-zero. To determine the asymmetry carried by the background on average, we can measure the asymmetry in the sidebands and extrapolate into the signal region.
...
\begin{figure}[H]
  ...
  \caption{ Linear fit to the background asymmetries measured in various invariant-mass sidebands from the $\pi^0$ peak.}
  \label{fig:bkgasy}
\end{figure}
...
\label{eqn:asybkgerr}
\end{equation}
Besides $A_{bkg}$, purity of the signal in our mass window is also needed. The purity equals the yield of signal over total yield. As mentioned before, the signal is fitted using both MC background method and Crystal Ball method. The discrepancy is taken as a source of systematic uncertainty.
...
\label{eqn:bkgcorr}
\end{equation}
P is the purity of the $\pi^0$ ($\eta$). Notice that the background asymmetry is actually close to the asymmetry measured in the  signal range. This dilutes the effect of the background correction. One explanation, referring to the decomposition in Fig.~\ref{fig:pi0_component}, is that the combinatorial background from different $\pi^0$ mesons in the same jet carries a significant Collins asymmetry.
...
\begin{figure}[H]
  ...
  \caption{$\pi^0$ double ratio asymmetry $\nicefrac{A^{\pi^0}_{12}}$ for different kinematic bins. The blue points are raw  asymmetries. The  pink points are asymmetries after background correction.}
  \label{fig:pi0afterbkg}
\end{figure}
...
\end{figure}
Similar to the $\pi^0$ case, the components of $\eta$ invariant mass distribution have been studied. In Fig.~\ref{fig:etacomponent} it can be seen that $\gamma$s and electrons originally coming from $\pi^0$ contribute the most to the background. The background asymmetry can be estimated using the same method that has been used with the $\pi^0$ background. Four sidebands are selected as $0.4638$~GeV$\textup{--}0.4878$~GeV, $0.4398$~GeV$\textup{--}0.4638$~GeV, $0.5838$~GeV$\textup{--}0.6078$~GeV and $0.6078$~GeV$\textup{--}0.6318$~GeV. We construct  the function $A_{bkg}(m)$ Vs invariant mass using a linear fit from which we estimate  $A_{bkg}(\eta  mass)$. The uncertainty of the background asymmetry is calculated through Eq.~\eqref{eqn:asybkgerr}.

As described in section~\ref{sec:etafitsection}, $\eta$ invariant mass can be fitted using two methods. One method is to fit the background with a polynomial function to the sidebands and then subtract the background from the total data to get the signal. The other one is to fit the sideband and signal region using a polynomial function plus Crystal Ball function. Here our purity is obtained through Crystal Ball fitting method.%To get the uncertainty of purity measurement, pol(6), pol(5) and pol(4) are used as fitting functions for first method. Crystal Ball function and pol(3) are used together as second method. 
...
\begin{figure}[H]
  ...
  \caption{$\eta$ double ratio asymmetry $\nicefrac{A^{\eta}_{12}}$ for different kinematic bins. The pink points are the originally measured raw asymmetries. Blue points are asymmetries after background correction.}
...
\begin{figure}[H]
 ...
 \subfigure[Collins angles based on {\em measured-thrust}]{\label{fig:afterillustr}\includegraphics[width=.9\textwidth,natwidth=600,natheight=400]{figure_theory/AfterSmear.pdf}}
  \caption{Illustration of the Collins angle before and after smearing. The {\em measured-thrust} does not align with the {\em stable-thrust} and the plane spanned by thrust and electrons is changed as shown in Fig.~\ref{fig:SmearIllustr}. During this smearing process, Collins angles are altered which leads to the dilution of Collins effect.}
...
A common method to evaluate smearing effects is to use re-weighting. Here we weight events in simulation according to their true kinematics and then estimate the effect of the smearing by comparing the reconstructed values with the input values.
 ...
 Here we re-weight our Monte Carlo simulation based on the distribution of hadron pairs about the {\em stable-thrust} axis. Based on the injected asymmetry $A_i$ we assign an event weight of $1+A_i*\cos\phi_{12}^{\text{stable}}$, where $\phi_{12}^\text{stable}$ is the Collins angle about the stable-thrust axis.

Collins asymmetries are then measured from an event-by-event weighted set using the measured Collins angle of all $\pi^0$ candidates in the signal window. Ideally if the thrust axis duplicates the stable thrust exactly, the injected asymmetry and the measured asymmetry should both be equal. However, if the thrust axis diverges from stable thrust, the Collins asymmetry is diluted thus leading to a lower measured asymmetry. The dilution ratio can then  be estimated to correct the measured asymmetry. Each kinematic bin will be corrected according to dilution of that bin.
...
\begin{figure}[H]
  ...
  \caption{weighted $\pi^0$ double ratio asymmetry $\nicefrac{A^{0\pm}_{12}}{A^L_{12}}$. As described in the text, the weight on the nominator  $\pi^0\pi^++\pi^0\pi^-$ is $1\times P_{t1}P_{t2}$ while the weight on the denominator $\pi^+\pi^++\pi^-\pi^- $ is $0.25\times P_{t1}P_{t2}$.}
  \label{fig:weightpi0}
\end{figure}

To also properly account for the bin migration mentioned before, the injected asymmetry has to be choosen such that it is similar (ideally identical) to the measured asymmetry after smearing. As explained above, this is mainly important for the $P_t$ shape. 
Fig.~\ref{fig:weightpi0} is an example of re-weighting asymmetries after smearing.
...
We adopted  a weight $1*P_{t1}*P_{t2}$ for the hadrons in the numerator and $0.25*P_{t1}*P_{t2}$ for hadrons in the denominator of the double ratio. 
 Since we measure double ratios, we can scale the weights in numerator and denominator to achieve the same measured asymmetry. We tried scaling up to a factor 5 and 
the difference in the resulting correction factors was negligible compared with their statistical uncertainties.
 The output based on {\em measured-thrust} is the smeared asymmetry and the output asymmetry based on {\em stable-thrust} is the real asymmetry that we manually generated. The ratio between those two is the smearing correction factor.
...
\end{tabular}
\caption{$A_\textrm{stable}$ is the asymmetry for that bin, measured using the correct 
...
the event-wise weights for that bin.
...
\begin{figure}[H]
  ...
  \caption{$\eta$ double ratio asymmetry $\nicefrac{A^{\eta\pm}_1}{A^L_1}$ for different kinematic bins. The triangle points are original measured asymmetry. Square points are asymmetries after background correction. Diamond points are asymmetries after background and thrust smearing correction, and the uncertainties include all systematics.}
  \label{fig:etaresult}
\end{figure}
